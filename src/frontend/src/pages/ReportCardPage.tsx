...
export default function ReportCardPage() {
  ...
  const handleSelectStudent = async (id: bigint) => {
    ...
    setMarks([]);
    try {
      const rc = await backend.getReportCard(id);
      if (rc) {
        setReportCard(rc);
        ...
        // Default template
        const defaultSubjects = ["English", "Mathematics", "Science", "Social Science", "Hindi"];
        const defaultColumns = ["FA1", "FA2", "SA1", "SA2", "Total"];
        const defaultMarks = defaultSubjects.map(() => defaultColumns.map(() => 0));
  ...
  const updateMark = (rowIdx: number, colIdx: number, value: string) => {
    ...
    setMarks((prev) =>
      prev.map((row, ri) =>
        ri === rowIdx ? row.map((cell, ci) => (ci === colIdx ? num : cell)) : row,
      ),
    );
  };
  ...
  const handleSave = async () => {
    if (!backend) { toast.error("Backend not ready"); return; }
    if (!selectedStudentId) { toast.error("No student selected"); return; }
    ...
    setSaving(true);
    try {
      await backend.saveOrUpdateReportCard(selectedStudentId, subjects, columns, marks);
      toast.success("Report card saved!");
      const rc = await backend.getReportCard(selectedStudentId);
  ...
  return (
    <div className="space-y-6">
      <div className="no-print">
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Create and manage student report cards</p>
      </div>
      ...
      <div className="no-print rounded-xl border border-border bg-card shadow-xs p-5">
        ...
        <select
          ...
          onChange={(e) => {
            if (e.target.value) handleSelectStudent(BigInt(e.target.value));
          }}
        ...
        {loadingStudents && <p className="text-xs text-muted-foreground mt-1">Loading students...</p>}
      </div>
      ...
      {selectedStudent && !loadingRC && subjects.length >= 0 && columns.length >= 0 && (
        <>
          {/* Print area header */}
          <div className="print-report-card">
            ...
            <div className="hidden print:block text-center border-b-2 border-gray-800 pb-4 mb-4">
              <div className="flex items-center justify-center gap-3 mb-2">
                <div className="w-14 h-14 rounded-full bg-green-800 flex items-center justify-center">
                  <span className="text-white font-extrabold text-sm">GPIS</span>
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-4 mt-3 text-sm text-left border border-gray-300 p-3 rounded">
                ...
                <div><span className="text-gray-500">Class:</span> <strong>{selectedStudent.studentClass} – {selectedStudent.section}</strong></div>
                <div><span className="text-gray-500">Father:</span> <strong>{selectedStudent.fatherName}</strong></div>
              </div>
            </div>
            ...
            {subjects.length === 0 || columns.length === 0 ? (
              <div className="rounded-xl border border-border bg-card shadow-xs py-12 text-center no-print">
                <p className="text-muted-foreground text-sm">Click "Add Subject" and "Add Column" to start building the report card.</p>
              </div>
            ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      ...
                      >
                        ...
                        {columns.map((col, ci) => (
                           <th key={`col-header-${ci}-${col}`} className="px-3 py-2 text-center min-w-24">
                            <div className="flex items-center gap-1 justify-center">
                              ...
                              />
                              <span className="print:inline hidden font-semibold text-xs text-foreground">{col}</span>
                              <button
                                type="button"
                    ...
                    <tbody className="divide-y divide-border">
                      {subjects.map((subject, ri) => (
                         <tr key={`row-${ri}-${subject}`} className="hover:bg-muted/10 transition-colors">
                          <td className="px-4 py-2">
                            <input
                              type="text"
                              value={subject}
                              onChange={(e) => updateSubjectName(ri, e.target.value)}
                            ...
                            />
                            <span className="print:inline hidden text-sm font-medium">{subject}</span>
                          </td>
                           {columns.map((col, ci) => (
                             <td key={`cell-${ri}-${ci}-${col}`} className="px-3 py-2 text-center">
                              <input
                                type="number"
                                value={marks[ri]?.[ci] ?? 0}
                                onChange={(e) => updateMark(ri, ci, e.target.value)}
                              ...
                              />
                              <span className="print:inline hidden text-sm font-medium">{marks[ri]?.[ci] ?? 0}</span>
                            </td>
                          ))}
      ...
      {!selectedStudentId && !loadingStudents && (
        <div className="rounded-xl border border-border bg-card shadow-xs py-16 text-center">
          ...
          <p className="text-muted-foreground text-sm">Select a student to view or create their report card</p>
        </div>
      )}
    </div>
  );
