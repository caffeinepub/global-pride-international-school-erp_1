...
import type { FeeCategory } from "../utils/localStore";
...
const EMPTY_FORM = {
  ...
  feePaymentCategory: FeeCategoryEnum.OTP,
  discountPercent: 0,
  examSA1: false,
  examSA2: false,
  dateOfBirth: "",
...
export default function StudentsPage() {
  ...
  const [extrasMap, setExtrasMap] = useState<Map<string, StudentExtras>>(
    new Map(),
  );
  ...
  const loadStudents = useCallback(() => {
    try {
      ...
      const extras = getAllStudentExtras();
      setStudents(data);
  ...
  const discountAmount = useMemo(
    () =>
      Math.round(
        (form.totalFee * Math.min(100, Math.max(0, form.discountPercent))) /
          100,
      ),
    ...
    try {
      if (editingId !== null) {
        updateStudent(editingId, {
          ...
          feePaymentCategory: form.feePaymentCategory,
          discountAmount,
          finalFee,
          examFeesSA1,
          examFeesSA2,
      ...
      toast.error(
        `Failed to save student: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  const handleDelete = (id: number) => {
    ...
    try {
      ...
      toast.error(
        `Failed to delete student: ${err instanceof Error ? err.message : String(err)}`,
      );
        ...
        <div className="rounded-xl border border-border bg-card shadow-xs overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.totalFee}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, totalFee: Number(e.target.value) }))
                  }
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                  min={0}
                />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      feePaymentCategory: e.target.value as FeeCategory,
                    }))
                  }
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                >
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="rounded-lg p-3 bg-secondary border border-border text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wide">
                  {feeCategoryInstallmentLabel(form.feePaymentCategory)}
                </p>
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setForm((p) => ({ ...p, examSA1: e.target.checked }))
                      }
                      className="rounded"
                    />
                    <span>SA1 ₹500</span>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setForm((p) => ({ ...p, examSA2: e.target.checked }))
                      }
                      className="rounded"
                    />
                    <span>SA2 ₹500</span>
                ...
                <tbody className="divide-y divide-border">
                  {filteredStudents.map((student) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell">
                        {extrasMap.get(student.id.toString())?.admissionNo || (
