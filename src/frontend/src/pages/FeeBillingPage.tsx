...
const PAYMENT_MODES = [
  ...
  { value: PaymentModeEnum.BankTransfer, label: "Bank Transfer" },
];
...
export default function FeeBillingPage() {
  ...
  const filteredStudents = useMemo(() => {
    let result = students;
    if (filterClass)
      result = result.filter((s) => s.studentClass === filterClass);
    if (filterSection)
  ...
  const handleSelectStudent = (student: Student) => {
    ...
    setAmount(
      String(
        getInstallmentAmount(student.finalFee, student.feePaymentCategory),
      ),
    );
  };
  ...
  const handlePay = () => {
    ...
    try {
      ...
      toast.success(
        `Payment of ${formatCurrency(amountNum)} recorded! Receipt: ${receiptNo}`,
      );
      // Refresh student data
      const refreshed = getStudentById(selectedStudent.id);
      if (refreshed) {
        ...
        // Update in the list
        setStudents((prev) =>
          prev.map((s) => (s.id === refreshed.id ? refreshed : s)),
        );
      }
      setAmount("");
    } catch {
            ...
            <div className="grid grid-cols-3 divide-x divide-border">
              <div className="px-5 py-4 text-center">
                ...
                <p className="text-xl font-bold text-foreground mt-1">
                  {formatCurrency(selectedStudent.finalFee)}
                </p>
              </div>
              <div className="px-5 py-4 text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wide">
              ...
              <div className="px-5 py-4 text-center">
                ...
                <p
                  className={`text-xl font-bold mt-1 ${remaining > 0 ? "text-amber-600" : "text-emerald-600"}`}
                >
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-emerald-700">
                    {formatCurrency(lastPayment.amountPaid)}
                  </p>
                </div>
                <div>
